...
func (s *SessionService) CreateSession(ctx context.Context, user *model.User, token, ipAddress, userAgent string, expiredAt time.Time) (*model.Session, error) {
...
func (s *SessionService) TerminateOtherSessions(ctx context.Context, userID string, currentSessionID string) error {
	...
	if err := db.Session(ctx).Select("token").
		...
		return fmt.Errorf("failed to query other sessions: %w", err)
	}
