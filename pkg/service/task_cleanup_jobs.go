...
func (s *taskService) runTaskLogCleanupJob(ctx context.Context, t *model.Task, progressCallback taskscheduler.ProgressCallback, cancelCh <-chan struct{}) (result interface{}, err error) {
	logger := log.GetContextLogger(ctx)
	retentionDays, _ := s.baseService.GetIntSetting(ctx, model.SettingTaskLogRetentionDays, 30)
	if retentionDays < 1 {
		retentionDays = 30
	}
