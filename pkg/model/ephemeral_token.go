...
// EphemeralTokenPurpose identifies why a short-lived token was issued.
...
// EphemeralToken stores short-lived, one-time-use tokens in the database so
// that OAuth state, MFA codes, and activation links work correctly across
// multiple backend instances.  The token value is never stored in plain
// text — only its SHA-256 hash is persisted.  Sensitive payload data is
// stored encrypted using the application encrypt-key.
...
func (EphemeralToken) TableName() string { return "t_ephemeral_token" }
