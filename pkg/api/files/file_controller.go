...
func (c *FileController) UploadFile(ctx *gin.Context) {
	...
	if err != nil {
		util.RespondWithError(ctx, util.NewErrorMessage("E4001", "file upload failed", err))
		return
	}

	if multipartForm == nil || len(multipartForm.File) == 0 {
		util.RespondWithError(ctx, util.NewErrorMessage("E4001", "file upload failed: file is required"))
		return
	}
